...
from harnesslens.evolution.rollout import RolloutModule
...
def main() -> int:
    ...
    result = RolloutModule(
        repo_root=root,
        run_root=run_root,
        budget=budget,
        harness=args.harness,
        cell=args.cell,
    ).run_from_main(main_decision=args.main_decision, label=args.label)
    print(json.dumps({"output": result.output_path, "metrics": result.output["metrics"], "budget": budget.status()}, ensure_ascii=False, indent=2, sort_keys=True))
    return 0
